...
test.describe('System Awakening — Prologue', () => {
  ...
  test('splash screen loads with New Game and Load Game buttons', async ({ page }) => {
    ...
    // BUG-TEST-2 fix: was 'System Awakening' but startup.txt sets game_title to
    // 'System: Year Zero'. The assertion must match the actual configured title.
  ...
  test('full prologue playthrough to ending', async ({ page }) => {
    ...
    // BUG-TEST-1 fix: pickChoice expects a string text fragment, not a Locator.
    // Passing page.locator('.choice-btn').first() caused Playwright to call
    // .toString() on the Locator, producing "Locator@..." which never matched
    // any button text and hung the test. Pass an empty string to match the
    // first available choice regardless of its label.
    ...
    // BUG-TEST-4 fix: prologue.txt ends with "To be continued..." and has no
    // *ending directive, so #ending-overlay never appears. The previous assertion
    //   await expect(endingOverlay).toBeVisible({ timeout: 10000 });
    //   await expect(page.locator('#ending-title')).toHaveText('The End');
    // always timed out. Instead verify the prologue's final narrative text is
    // visible, confirming the scene ran to completion.
    // NOTE: If you add *ending to prologue.txt, replace this with the overlay check.
    await expect(
  ...
  // BUG-TEST-3: This test was removed. It asserted that pressing the backtick
  // key toggled a #debug-overlay element, but neither #debug-overlay nor
  // .debug-header exist in index.html, and no JS file registers a backtick
  // keydown handler. The test always failed immediately with a "strict mode
  // violation" locator error. Re-implement once the debug overlay feature is
  // built and wired in engine.js / index.html.
  test.skip('debug overlay toggles with backtick', async () => {});
